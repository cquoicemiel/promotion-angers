...
const Part = (props) => {
  return (
    <div
      style={
        props.state ? { left: 0, opacity: 1 } : { left: "-100%", opacity: 0 }
      }
      className={`w-screen h-screen fixed bg-white top-0 bottom-0 transition-all z-30 overflow-y-scroll px-12 py-8`}
    >
      <div className="h-24 flex justify-around items-center">
        <span className="basis-5/6 flex items-center pl-8 text-3xl xl:text-4xl font-semibold capitalize">
          {props.title}
        </span>
        <Image
          onClick={() => {
          ...
          className={
            "rotate-180 contain h-10 w-10 cursor-pointer box-content p-1 hover:invert hover:border-white bg-white transition-all border-2 border-black rounded-xl"
          }
...
const Card = (props) => {
  return (
    <div
      style={{ minHeight: "calc(24rem + 5vw)" }}
      onClick={() => {
        props.setState(true);
      }}
      className="card rounded-2xl w-full h-full w-1/4 shadow-xl flex flex-col px-8 cursor-pointer shadow-md hover:shadow-xl transition-all hover:bg-gray-100 bg-white z-10 justify-around hover:z"
    >
      <span className="capitalize basis-1/4 flex justify-center items-center text-2xl font-bold text-center">
        {props.title}
      </span>
      <span className="basis-1/2 text-xl text-start selection:bg-green-400 selection:text-white">
        {props.text}
      </span>
      <span className="flex justify-center items-center basis-1/4">
        <span className="text-xl -translate-y-0.5">En savoir plus</span>
        <Image
          className="arrow transition-all cover h-7 w-7 opacity-60"
          alt={"En savoir plus"}
          src={require("../assets/arrow.png")}
        />
      </span>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      ...
      <div
        ...
        className="flex relative w-full px-6 xl:px-48 py-12 justify-around z-20"
      >
        <Card
          state={first}
          setState={setFirst}
        ...
        <Image
          ...
          } fixed right-8 bottom-8 self-center cursor-pointer h12 w-12 transition-all animate-pulse hover:animate-none bg-gray-300 rounded-full p-2`}
          src={require("../assets/arrow.png")}
          alt={"Descendre"}
        />
      </div>
      <span className="py-16 w-full justify-center flex flex-col items-center">
        ...
        <div className="text-gray-500 text-2xl w-1/2 selection:bg-green-400 selection:text-white">
          <span className="font-semibold">
            {
              "Je vous résume les 5 raisons principales de vivre à Angers en 5 minutes:"
            }
          </span>
          <br />
          <br />
